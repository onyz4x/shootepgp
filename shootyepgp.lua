...
function sepgp:OnEnable() -- PLAYER_LOGIN (2)
  ...
  self:RegisterEvent("CHAT_MSG_RAID","captureBid")
  self:RegisterEvent("CHAT_MSG_RAID_LEADER","captureBid")
...
function sepgp:captureLootCall(text, sender)
  ...
  if (whisperkw_found) or (mskw_found) or (oskw_found) then
    -- _,_,itemLink = string.find(text, "|c%x+|H+|h|r")
    _,_,itemLink = string.find(text, "发起(.+)的竞拍")
